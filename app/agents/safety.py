...
def safety_node(state: WorkflowState) -> dict:
    ...
    def escalate_request(reason: str) -> dict:
        """Escalate this request for human review, because it describes a
        possible medical emergency or a sensitive situation a human should
        handle directly."""
    ...
    # SafetyAllow — the request is safe; step out of the way and let the graph
    # continue to whatever the Coordinator already delegated to.
    update_workflow_run(
        workflow_run_id,
        current_step="safety",
